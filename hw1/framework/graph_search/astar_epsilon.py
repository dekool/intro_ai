...
class AStarEpsilon(AStar):
    ...
    def _extract_next_search_node_to_expand(self, problem: GraphProblem) -> Optional[SearchNode]:
        ...
        while not self.open.is_empty():
            ...
            if (self.max_focal_size is None or len(focal) < self.max_focal_size) and node.expanding_priority <= max_expanding_priority:
                focal.append(node)
            else:
                self.open.push_node(node)
                break  # the open is sorted, so once we reached a node not in scope - we can break

        focal_values = []
        for node in focal:
            focal_values.append(self.within_focal_priority_function(node, problem, self))
        focal_min_index = np.argmin(focal_values)
        chosen = focal[focal_min_index.__int__()]
        for node in focal:
            if node != chosen:
                self.open.push_node(node)
        if self.use_close:
