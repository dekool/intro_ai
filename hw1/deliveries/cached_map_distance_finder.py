...
class CachedMapDistanceFinder:
    ...
    def get_map_cost_between(self, src_junction: Junction, tgt_junction: Junction) -> Optional[Cost]:
        ...
        problem = MapProblem(self.streets_map, src_junction.index, tgt_junction.index, self.road_cost_fn, self.zero_road_cost)
        solution = self.map_problem_solver.solve_problem(problem)
        self._insert_to_cache(key, solution.solution_cost if solution.is_solution_found else None)
        return solution.solution_cost if solution.is_solution_found else None
