...
class TruckDeliveriesMaxAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        total_distance_lower_bound = max(self.cached_air_distance_calculator.get_air_distance_between_junctions(junction1, junction2)
...
class TruckDeliveriesSumAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        last_junction_in_path = state.current_location
        # remove the current location
        all_junctions_in_remaining_truck_path.remove(last_junction_in_path)
        while len(all_junctions_in_remaining_truck_path) > 0:
            cur_nearest = [None, None]  # hold distance and the junction object
            # calculate the path length between the last junction and each other junction
            for junction in all_junctions_in_remaining_truck_path:
                if junction != last_junction_in_path:
                    temp_distance = self.cached_air_distance_calculator.get_air_distance_between_junctions(last_junction_in_path, junction)
                    # keep the minimum distance found
                    if cur_nearest[0] is None or temp_distance < cur_nearest[0]:
                        cur_nearest[0] = temp_distance
                        cur_nearest[1] = junction
            # add the minimum distance to the total distance, and continue to the next junction
            last_junction_in_path = cur_nearest[1]
            total_cost_of_greedily_built_path += cur_nearest[0]
            all_junctions_in_remaining_truck_path.remove(cur_nearest[1])
...
class TruckDeliveriesMSTAirDistHeuristic(HeuristicFunction):
    ...
    def _calculate_junctions_mst_weight_using_air_distance(self, junctions: Set[Junction]) -> float:
        ...
        """
        graph = nx.Graph()
        # create the nodes with the edges. it is a full graph - there is an edge between each pair on junctions
        junctions_data = [(junction.index, other_junction.index,
                           self.cached_air_distance_calculator.get_air_distance_between_junctions(junction, other_junction))
                          for junction in junctions
                          for other_junction in junctions
                          if junction != other_junction]
        # add the nodes and edges to the graph
        graph.add_weighted_edges_from(junctions_data)
