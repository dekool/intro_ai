...
class DeliveriesTruckProblem(GraphProblem):
    ...
    def expand_state_with_costs(self, state_to_expand: GraphProblemState) -> Iterator[OperatorResult]:
        ...
        truck_loaded_packages = state_to_expand.get_total_nr_packages_loaded()
        max_truck_size = self.problem_input.delivery_truck.max_nr_loaded_packages

        for delivery in waiting_to_pick:
            # filter out the deliveries we can't pick because of truck size
            if max_truck_size >= truck_loaded_packages + delivery.nr_packages:
                new_loaded = frozenset(state_to_expand.loaded_deliveries.union({delivery}))
                successor = DeliveriesTruckState(new_loaded, state_to_expand.dropped_deliveries, delivery.pick_location)
                cost = self.map_distance_finder.get_map_cost_between(state_to_expand.current_location, delivery.pick_location)
                yield OperatorResult(successor_state=successor, operator_cost=cost, operator_name="pick "+delivery.client_name)

        for delivery in state_to_expand.loaded_deliveries:
            new_dropped = frozenset(state_to_expand.dropped_deliveries.union({delivery}))
            new_loaded = frozenset(state_to_expand.loaded_deliveries.difference({delivery}))
            successor = DeliveriesTruckState(new_loaded, new_dropped, delivery.drop_location)
            cost = self.map_distance_finder.get_map_cost_between(state_to_expand.current_location, delivery.drop_location)
            yield OperatorResult(successor_state=successor, operator_cost=cost, operator_name="drop "+delivery.client_name)
    ...
    def is_goal(self, state: GraphProblemState) -> bool:
        ...
        return len(self.problem_input.deliveries) == len(state.dropped_deliveries)
    ...
    def _calc_map_road_cost(self, link: Link) -> DeliveryCost:
        ...
        return DeliveryCost(
            ...
            money_cost=link.distance*(gas_cost_per_meter+link.is_toll_road*self.problem_input.toll_road_cost_per_meter),
    ...
    def get_cost_lower_bound_from_distance_lower_bound(self, total_distance_lower_bound: float) -> float:
        ...
        if self.optimization_objective == OptimizationObjective.Distance:
            ...
            assert self.optimization_objective == OptimizationObjective.Money
            remaining_path_link = Link(0, 1, total_distance_lower_bound, 0, MAX_ROAD_SPEED)
            return self._calc_map_road_cost(remaining_path_link).money_cost
    ...
    def get_all_junctions_in_remaining_truck_path(self, state: DeliveriesTruckState) -> Set[Junction]:
        ...
        """
        waiting_pick_junctions = {delivery.pick_location for delivery in self.get_deliveries_waiting_to_pick(state)}
        waiting_drop_junctions = {delivery.drop_location for delivery in self.get_deliveries_waiting_to_pick(state)}
        loaded_drop_junction = {delivery.drop_location for delivery in state.loaded_deliveries}
        waiting_all_junctions = waiting_pick_junctions.union(waiting_drop_junctions)
        return waiting_all_junctions.union(loaded_drop_junction.union({state.current_location}))
